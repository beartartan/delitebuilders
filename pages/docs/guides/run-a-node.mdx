...
This guide will walk you through the process of operating a Delite node via [simple-delite-node](https://github.com/delitesystems/simple-delite-node). You will be able to:
...
<Steps>
### Clone simple-delite-node
```sh
git clone https://github.com/delitesystems/simple-delite-node.git
cd simple-delite-node
```
...
Update the `simple-delite-node` Docker images:
...
```sh
docker compose logs -f delite_client_proposer
```

#### View the prover image's logs

```sh
docker compose logs -f delite_client_prover_relayer
```
...
This error just means that the port delite-node is trying to allocate is already being used by either some other node you're running
...
You are in the wrong directory, `cd simple-delite-node` and run the command again.
...
#### `delite_client_prover_relayer not printing any logs`
...
#### `simple-delite-node-delite_client_prover_relayer-1  | /bin/sh: /script/start-prover-relayer.sh: not found`
